...
@st.cache_resource
def get_tf_modules():
    """Lazily load TensorFlow/Keras only when needed."""
...
try:
    ...
    with c2:
        st.markdown("<h1 style='padding-top: 10px;'>F1 RACE ENGINEER AI <span style='font-size: 0.5em; color: #ff2800;'>PRO v2.2</span></h1>", unsafe_allow_html=True)
    ...
    if selection == "Syntax Analysis":
        # Load spaCy lazily
        with st.spinner("INITIALIZING SYNTAX PARSER..."):
            nlp = load_nlp_core()
        ...
        with st.spinner("LOADING NER PROTOCOLS..."):
            nlp = load_nlp_core()
            from spacy import displacy
        ...
        with st.spinner("CALIBRATING SEMANTIC MODELS..."):
            nlp = load_nlp_core()
        ...
        with st.spinner("Initializing Neural Core (Loading TensorFlow may take a moment)..."):
            model, tokenizer = train_production_model()
            # Need to get pad_sequences for prediction
            _, _, pad_sequences, _ = get_tf_modules()
    ...
    # GLOBAL ERROR HANDLER FOR CLOUD DEBUGGING
    st.error("🚨 CRITICAL SYSTEM FAILURE 🚨")
    st.error(f"Error Code: {str(e)}")
    with st.expander("VIEW BLACK BOX DATA (TRACEBACK)"):
